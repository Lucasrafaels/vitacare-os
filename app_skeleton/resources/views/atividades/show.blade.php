...
<div class="topbar">
    ...
    <span class="topbar-title" style="margin-left:12px">{{ $atv->nome }}</span>
    <div class="topbar-actions">
        <a href="/atividades/{{ $atv->id }}/editar" class="btn btn-outline btn-sm">Editar</a>
        <form method="POST" action="/atividades/{{ $atv->id }}/excluir"
              style="display:inline" onsubmit="return confirm('Desativar esta atividade?')">
            @csrf
            ...
            <div class="card">
                <div class="card-body" style="padding:24px 20px">
                    ...
                    <div style="margin-top:12px">
                        <span class="badge {{ $atv->status === 'ativa' ? 'badge-green' : 'badge-gray' }}">
                            {{ ucfirst($atv->status) }}
                        </span>
                    </div>
                </div>
        ...
        <div>
            <div class="card">
                <div class="card-header" style="display:flex;align-items:center;justify-content:space-between">
                    ...
                    <a href="/os?atividade={{ $atv->id }}" class="btn btn-outline btn-sm">Ver todas</a>
                </div>
                @if ($ultimas->isEmpty())
                <div class="card-body">
                    <div class="empty-state">Nenhuma OS registrada para esta atividade.</div>
                </div>
                @else
                <div class="table-wrap">
                    <table>
                        <thead><tr><th>Código</th><th>Profissional</th><th>Unidade</th><th>Data</th><th>Status</th></tr></thead>
                        <tbody>
                        ...
                        @endphp
                        <tr>
                            <td><a href="/os/{{ $os->id }}"><code style="font-size:12px">{{ $os->codigo }}</code></a></td>
